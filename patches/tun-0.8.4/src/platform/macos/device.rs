...
fn ipv4_mask_to_prefix(mask: Ipv4Addr) -> Result<u8> {
    let mut prefix = 0u8;
    ...
    for octet in mask.octets() {
        if octet == 0xFF {
            ...
            prefix = prefix.saturating_add(8);
            continue;
        }
        if octet == 0 {
            seen_zero = true;
        ...
        if seen_zero || octet.count_ones() + octet.leading_ones() != 8 {
            return Err(Error::InvalidConfig);
        }
        prefix = prefix.saturating_add(octet.count_ones() as u8);
        seen_zero = true;
    }
    Ok(prefix)
}
...
impl Device {
    ...
    fn set_route(&mut self, route: Route) -> Result<()> {
        if let Some(v) = &self.route {
            let prefix_len = ipv4_mask_to_prefix(v.netmask)?;
        ...
        let prefix_len = ipv4_mask_to_prefix(route.netmask)?;
        let args = [
            "-n",
            "add",
            "-net",
